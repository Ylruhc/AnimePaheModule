...
async function searchResults(keyword) {
    ...
    try {
        ...
        const response = await fetch(`${ SEARCH_URL }/?m=search&q=${ encodeURIComponent(keyword) }`);
...
async function fetchDownloadLink(downloadPageLink) {
    ...
    const pageResponse = await fetch(downloadPageLink, {
        method: "GET",
        headers: { 
    ...
    const contentResponse = await fetch(actionUrl, {
        method: 'POST',
        headers: {
...
searchResults('Hajime no Ippo: New Challenger').then(
    ...
    extractEpisodes(jsonObj[0]['href']).then((x)=>
        {
            extractStreamUrl(JSON.parse(x)[0]['href']).then(console.log)
        })
    }
)